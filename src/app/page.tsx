...
import { calculateTransits, getRotatedChart, PlanetData, DivisionalChartData, Predictions, DashaInfo, SadeSatiInfo, Remedy, AyanamsaType, AshtakavargaData, generateSoulPurpose, SoulPurpose } from "@/lib/astrology";
...
export default function Home() {
  ...
  // UI State
  const [soulPurpose, setSoulPurpose] = useState<SoulPurpose | null>(null);
    ...
    const timer = setTimeout(() => {
      ...
      queueMicrotask(() => {
        ...
        if (birthDetails) {
            ...
            setSoulPurpose(generateSoulPurpose(natal.planets));
        }
      });
  ...
  return (
    <main className="min-h-screen flex flex-col pb-32 bg-[#FDF8ED] text-[#2D3A3A] font-sans">
      <div className="flex-1 max-w-7xl mx-auto w-full p-4 md:p-8 space-y-12 relative">

        {/* Global Input Section (Collapsible) */}
        <section className="bg-white rounded-3xl shadow-sm border border-[#2D3A3A]/10 overflow-hidden transition-all duration-500 ease-in-out" ref={suggestionRef}>
          {!isSettingsOpen ? (
            <div className="p-4 md:p-6 flex flex-col md:flex-row items-center justify-between gap-4 bg-gradient-to-r from-white to-[#FDF8ED] select-none">
              <div className="flex flex-wrap items-center gap-6 text-sm">
                <div className="flex items-center gap-2">
                  <div className="w-8 h-8 rounded-full bg-[#D97757]/10 flex items-center justify-center">
                    <User aria-hidden="true" className="w-4 h-4 text-[#D97757]" />
                  </div>
                  <div>
                    <div className="text-[10px] font-bold text-[#2D3A3A]/40 uppercase tracking-widest">Natal</div>
                    <div className="font-semibold">{birthPob.split(',')[0]} • {birthDateStr}</div>
                  </div>
                </div>
                <div className="hidden md:block h-8 w-px bg-[#2D3A3A]/10" />
                <div className="flex items-center gap-2">
                  <div className="w-8 h-8 rounded-full bg-[#2D3A3A]/10 flex items-center justify-center">
                    <Navigation aria-hidden="true" className="w-4 h-4 text-[#2D3A3A]" />
                  </div>
                  <div>
                    <div className="text-[10px] font-bold text-[#2D3A3A]/40 uppercase tracking-widest">Transit</div>
              ...
              <button
                ...
                className="flex items-center gap-2 px-4 py-2 bg-[#2D3A3A] text-white rounded-full text-xs font-bold hover:bg-[#2D3A3A]/90 transition-all shadow-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-offset-[#FDF8ED] focus-visible:ring-[#D97757] select-none"
              >
            ...
            <div className="p-6">
              <div className="flex justify-between items-center mb-8 pb-4 border-b border-[#2D3A3A]/5 select-none">
                <h2 className="text-2xl font-serif flex items-center gap-3">
                  <Settings aria-hidden="true" className="w-6 h-6 text-[#D97757]" /> Configuration
                </h2>
                <button
                  onClick={() => setIsSettingsOpen(false)}
                  className="p-2 hover:bg-[#FDF8ED] rounded-full transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#D97757] select-none"
              ...
              <div className="flex flex-col lg:flex-row gap-8 relative">
                ...
                <div className="flex-1 space-y-6 lg:pr-8 lg:border-r border-[#2D3A3A]/10">
                  <h3 className="text-sm font-bold text-[#D97757] uppercase tracking-[0.2em]">Birth Details</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-2 col-span-1 md:col-span-2 relative">
                      <label htmlFor="birthCity" className="text-xs font-bold text-[#2D3A3A]/40 uppercase flex items-center gap-2 cursor-pointer hover:text-[#2D3A3A]/60 transition-colors select-none">
                      ...
                      <div className="relative">
                        <input
                          ...
                          className="w-full p-3 bg-[#FDF8ED] border border-[#2D3A3A]/10 rounded-xl outline-none focus:ring-2 focus:ring-[#D97757]/20 transition-all"
                          placeholder="Search birth city..."
                        />
                        {isLoadingCity && showSuggestionsFor === "birth" && <div role="status" aria-label="Loading cities..." className="absolute right-3 top-3.5 animate-spin w-4 h-4 border-2 border-[#D97757] border-t-transparent rounded-full" />}

                    {/* Birth Suggestions Dropdown */}
                    {showSuggestionsFor === "birth" && suggestions.length > 0 && (
                      <div id="birth-suggestions-listbox" role="listbox" className="absolute z-50 w-full mt-1 bg-white border border-[#2D3A3A]/10 rounded-xl shadow-xl overflow-hidden max-h-60 overflow-y-auto shadow-[#2D3A3A]/20 top-full left-0 select-none">
                        {suggestions.map((s, i) => (
                          <button
                            key={i}
                            type="button"
                            ...
                            className={`w-full text-left px-4 py-3 text-sm border-b border-[#2D3A3A]/5 last:border-0 transition-colors focus-visible:outline-none focus-visible:bg-[#D97757]/10 select-none ${i === activeSuggestionIndex ? 'bg-[#D97757]/10 text-[#D97757] font-bold' : 'text-[#2D3A3A]/80 hover:bg-[#FDF8ED]'}`}
                          >
                            {s.name}
                          </button>
                        ))}
                    ...
                    <div className="space-y-2">
                      <label htmlFor="birthDate" className="text-xs font-bold text-[#2D3A3A]/40 uppercase flex items-center gap-2 cursor-pointer hover:text-[#2D3A3A]/60 transition-colors select-none"><Calendar aria-hidden="true" className="w-3.5 h-3.5"/> Date</label>
                      <input id="birthDate" type="date" value={birthDateStr} onChange={e => setBirthDateStr(e.target.value)} className="w-full p-3 bg-[#FDF8ED] border border-[#2D3A3A]/10 rounded-xl outline-none focus:ring-2 focus:ring-[#D97757]/20 transition-all" />
                    </div>
                    <div className="space-y-2">
                      <label htmlFor="birthTime" className="text-xs font-bold text-[#2D3A3A]/40 uppercase flex items-center gap-2 cursor-pointer hover:text-[#2D3A3A]/60 transition-colors select-none"><Clock aria-hidden="true" className="w-3.5 h-3.5"/> Time</label>
                      <input id="birthTime" type="time" value={birthTimeStr} onChange={e => setBirthTimeStr(e.target.value)} className="w-full p-3 bg-[#FDF8ED] border border-[#2D3A3A]/10 rounded-xl outline-none focus:ring-2 focus:ring-[#D97757]/20 transition-all" />
                    </div>
                  </div>
                </div>
                ...
                <div className="flex-1 space-y-6 select-none">
                   <h3 className="text-sm font-bold text-[#2D3A3A]/40 uppercase tracking-[0.2em]">Transit Parameters</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-2 col-span-1 md:col-span-2 relative">
                      <label htmlFor="transitCity" className="text-xs font-bold text-[#2D3A3A]/40 uppercase flex items-center gap-2 cursor-pointer hover:text-[#2D3A3A]/60 transition-colors select-none">
                      ...
                      <div className="relative">
                        <input
                          ...
                          className="w-full p-3 bg-[#FDF8ED] border border-[#2D3A3A]/10 rounded-xl outline-none focus:ring-2 focus:ring-[#2D3A3A]/20 transition-all"
                          placeholder="Search transit city..."
                        />
                        {isLoadingCity && showSuggestionsFor === "transit" && <div role="status" aria-label="Loading cities..." className="absolute right-3 top-3.5 animate-spin w-4 h-4 border-2 border-[#D97757] border-t-transparent rounded-full" />}

                    {/* Transit Suggestions Dropdown */}
                    {showSuggestionsFor === "transit" && suggestions.length > 0 && (
                      <div id="transit-suggestions-listbox" role="listbox" className="absolute z-50 w-full mt-1 bg-white border border-[#2D3A3A]/10 rounded-xl shadow-xl overflow-hidden max-h-60 overflow-y-auto shadow-[#2D3A3A]/20 top-full left-0 select-none">
                        {suggestions.map((s, i) => (
                          <button
                            key={i}
                            type="button"
                    ...
                    <div className="space-y-2">
                      <label htmlFor="transitDate" className="text-xs font-bold text-[#2D3A3A]/40 uppercase flex items-center gap-2 cursor-pointer hover:text-[#2D3A3A]/60 transition-colors select-none"><Calendar aria-hidden="true" className="w-3.5 h-3.5"/> Date</label>
                      <input id="transitDate" type="date" value={transitDateStr} onChange={e => setTransitDateStr(e.target.value)} className="w-full p-3 bg-[#FDF8ED] border border-[#2D3A3A]/10 rounded-xl outline-none focus:ring-2 focus:ring-[#2D3A3A]/20 transition-all" />
                    </div>
                    <div className="space-y-2">
                      <label htmlFor="transitTime" className="text-xs font-bold text-[#2D3A3A]/40 uppercase flex items-center gap-2 cursor-pointer hover:text-[#2D3A3A]/60 transition-colors select-none"><Clock aria-hidden="true" className="w-3.5 h-3.5"/> Time</label>
                      <input id="transitTime" type="time" value={transitTimeStr} onChange={e => setTransitTimeStr(e.target.value)} className="w-full p-3 bg-[#FDF8ED] border border-[#2D3A3A]/10 rounded-xl outline-none focus:ring-2 focus:ring-[#2D3A3A]/20 transition-all" />
                    </div>
                  </div>
                </div>
              </div>
        ...
        {activeTab === "dashboard" && (
          <div className="space-y-8 animate-in fade-in duration-500">
            <header className="flex flex-col md:flex-row justify-between items-center gap-4 select-none">
              <h1 className="text-4xl font-serif text-[#2D3A3A] tracking-tight select-none">Your Personal Energy Forecast</h1>
              <div className="flex flex-wrap gap-4">
                <div className="flex bg-white rounded-lg border border-[#2D3A3A]/10 p-1 shadow-sm" role="group" aria-label="Select Ayanamsa">
                  {["Lahiri", "Raman", "Fagan-Bradley"].map(a => <button key={a} aria-pressed={ayanamsa === a} onClick={() => setAyanamsa(a as AyanamsaType)} className={`px-3 py-1 text-xs rounded-md transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#D97757] ${ayanamsa === a ? 'bg-[#D97757] text-white font-bold' : 'text-[#2D3A3A]/60 hover:bg-[#FDF8ED]'}`}>{a}</button>)}
                </div>
                <div className="flex bg-white rounded-lg border border-[#2D3A3A]/10 p-1 shadow-sm" role="group" aria-label="Select Reference Point">
                  {["Moon", "Lagna", "Dasha Lord"].map(r => <button key={r} aria-pressed={referencePoint === r} onClick={() => setReferencePoint(r as "Moon" | "Lagna" | "Dasha Lord")} className={`px-3 py-1 text-xs rounded-md transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#D97757] ${referencePoint === r ? 'bg-[#2D3A3A] text-white font-bold' : 'text-[#2D3A3A]/60 hover:bg-[#FDF8ED]'}`}>{r}</button>)}
                </div>
              </div>
            </header>

            <section className="flex flex-col items-center justify-center p-12 bg-white rounded-[3rem] border border-[#2D3A3A]/10 shadow-xl relative overflow-hidden select-none">
                {sadeSati?.isActive && (
                ...
                <div className="mb-10 text-center space-y-2">
                  <h2 className="text-sm font-bold text-[#2D3A3A]/40 uppercase tracking-[0.3em]">Energy Forecast</h2>
                  <p className="text-2xl font-serif text-[#2D3A3A]">
                ...
                <div className="relative w-64 h-64 flex items-center justify-center">
                    <svg role="img" aria-label="Transit Score Radial Graph" className="w-full h-full transform -rotate-90" viewBox="0 0 100 100">
                        <circle cx="50" cy="50" r="45" fill="none" stroke="#FDF8ED" strokeWidth="10" />
                        <circle cx="50" cy="50" r="45" fill="none" stroke="#D97757" strokeWidth="10" strokeDasharray="283" strokeDashoffset={283 - (283 * gocharaScore) / 100} className="transition-all duration-1000 ease-out" strokeLinecap="round" />
                    </svg>
                    <div className="absolute flex flex-col items-center">
                        <span className="text-7xl font-serif font-bold text-[#2D3A3A] tracking-tighter">{gocharaScore.toFixed(1)}<span className="text-2xl text-[#2D3A3A]/40 ml-1">%</span></span>
                ...
                {dasha && (
                  <div className="mt-12 w-full max-w-md select-none">
                    <div className="flex items-center gap-4 mb-4">
                      <div className="h-px flex-1 bg-gradient-to-r from-transparent to-[#2D3A3A]/10" />
                      <span className="text-[10px] font-bold text-[#2D3A3A]/30 uppercase tracking-[0.2em]">Active Influence</span>
                      <div className="h-px flex-1 bg-gradient-to-l from-transparent to-[#2D3A3A]/10" />
                    </div>
                    <div className="text-2xl text-[#2D3A3A] font-serif text-center flex flex-wrap justify-center items-center gap-1">
                      <span className="text-[#D97757]" title="Mahadasha">{dasha.mahadasha.lord}</span>
                      <span className="text-[#2D3A3A]/20">/</span>
                      <span title="Antardasha">{dasha.antardasha.lord}</span>
                      <span className="text-[#2D3A3A]/20">/</span>
                      <span className="text-sm text-[#2D3A3A]/80" title="Pratyantardasha">{dasha.pratyantardasha.lord}</span>
                      <span className="text-[#2D3A3A]/20">/</span>
            ...
            <section className="bg-white p-8 rounded-[3rem] border border-[#2D3A3A]/10 shadow-xl space-y-6 select-none relative overflow-hidden">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-center border-b border-[#2D3A3A]/5 pb-6">
                    <div className="space-y-1">
                        <div className="text-xs font-bold text-[#2D3A3A]/40 uppercase tracking-widest flex items-center gap-2">
                            Current Position
                            {scrubDays !== 0 && (
                                <button
                                    onClick={() => setScrubDays(0)}
                                    aria-label="Reset to current date"
                                    className="bg-[#D97757]/15 text-[#D97757] px-2 py-0.5 rounded text-[10px] font-extrabold hover:bg-[#D97757]/25 transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#D97757]"
                                >
                                    Reset to Today
                                </button>
                            )}
                        </div>
                        <div className="text-2xl font-serif text-[#2D3A3A]">{getScrubbedDateStr()}</div>
                        <div className="text-xs text-[#2D3A3A]/50">
                    ...
                    <div className="flex md:justify-center items-center gap-4">
                        <div className="w-12 h-12 rounded-2xl bg-[#D97757]/10 flex items-center justify-center font-serif text-xl font-bold text-[#D97757]">
                            {gocharaScore.toFixed(0)}
                        </div>
                        <div>
                            <div className="text-xs font-bold text-[#2D3A3A]/40 uppercase tracking-widest">Gochara Score</div>
                    ...
                    <div className="flex flex-col md:items-end justify-center space-y-1">
                        <div className="text-xs font-bold text-[#2D3A3A]/40 uppercase tracking-widest">Active Sign Transits</div>
                        <div className="flex flex-wrap md:justify-end gap-1.5 max-w-xs">
                            {activeSignEvents.length > 0 ? (
                                activeSignEvents.map((ev, i) => (
                                    <span key={i} className="text-[10px] font-bold bg-[#D97757] text-white px-2.5 py-1 rounded-full shadow-sm animate-bounce">
                                        {ev.label}
                                    </span>
                                ))
                            ) : (
                                <span className="text-xs text-[#2D3A3A]/40 italic">No exact sign changes today</span>
                            )}
                        </div>
                    </div>
                </div>
                ...
                <div className="relative">
                    <div className="absolute inset-y-0 left-0 flex flex-col justify-between pointer-events-none text-[10px] font-bold text-[#2D3A3A]/30 uppercase pl-1 select-none py-2">
                        <div>Excellent</div>
                        <div>Neutral</div>
                        <div>Caution</div>
                    </div>

                    <div className="h-56 w-full bg-gradient-to-b from-[#FDF8ED]/50 to-white rounded-2xl border border-[#2D3A3A]/5 relative overflow-hidden select-none">
                        <svg
                            ref={svgRef}
                            role="img"
                        ...
                        >
                            <defs>
                                <linearGradient id="timeline-gradient" x1="0" y1="0" x2="0" y2="1">
                                    ...
                                    <stop offset="100%" stopColor="#D97757" stopOpacity="0.00" />
                                </linearGradient>
                            </defs>

                            <line x1="0" y1="50" x2="100" y2="50" stroke="#2D3A3A" strokeWidth="1" strokeDasharray="3 3" strokeOpacity="0.12" vectorEffect="non-scaling-stroke" />
                            <line x1="0" y1="30" x2="100" y2="30" stroke="#2D3A3A" strokeWidth="1" strokeDasharray="2 2" strokeOpacity="0.04" vectorEffect="non-scaling-stroke" />
                            <line x1="0" y1="70" x2="100" y2="70" stroke="#2D3A3A" strokeWidth="1" strokeDasharray="2 2" strokeOpacity="0.04" vectorEffect="non-scaling-stroke" />
                        ...
                        <div
                            className="absolute w-3 h-3 rounded-full bg-white border-2 border-[#D97757] shadow-[0_0_8px_#D97757] transition-all duration-75 pointer-events-none"
                            style={{
                ...
                <div className="space-y-1">
                    <input
                        ...
                        className="w-full h-1.5 bg-[#FDF8ED] rounded-lg appearance-none cursor-pointer accent-[#D97757] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#D97757]"
                    />
                    <div className="flex justify-between text-[10px] font-bold text-[#2D3A3A]/40 uppercase tracking-widest select-none">
                        <span>-30 Days</span>
                        <span>Today</span>
                        <span>+30 Days</span>
                    </div>
        ...
        {activeTab === "predictions" && (
          <div className="space-y-8 animate-in fade-in duration-500">
            ...
            <div className="bg-gradient-to-r from-[#2D3A3A] to-[#254F56] text-white p-8 md:p-10 rounded-[2.5rem] shadow-xl space-y-4 select-none relative overflow-hidden">
              ...
              <div className="flex flex-col sm:flex-row gap-4 pt-4 border-t border-white/10 items-start sm:items-center justify-between">
                <div className="flex items-center gap-2 text-xs text-[#D97757] font-bold uppercase tracking-wider">
                ...
                <div className="flex gap-2 bg-white/10 p-1 rounded-lg">
                  <button
                    ...
                    className={`px-4 py-1.5 text-xs rounded-md transition-all font-bold ${predictionsViewMode === "summary" ? 'bg-[#D97757] text-white shadow' : 'text-white/60 hover:text-white'}`}
                  >
                    Summary View
                  </button>
                  <button
                    onClick={() => setPredictionsViewMode("detailed")}
                    className={`px-4 py-1.5 text-xs rounded-md transition-all font-bold ${predictionsViewMode === "detailed" ? 'bg-[#D97757] text-white shadow' : 'text-white/60 hover:text-white'}`}
                  >
                    Detailed View
                  </button>
                </div>
            ...
            <header className="flex flex-col md:flex-row justify-between items-center gap-4 select-none">
              <h2 className="text-2xl font-serif text-[#2D3A3A] tracking-tight font-bold">Configure Predictive Baseline</h2>
              <div className="flex flex-wrap gap-4">
                <div className="flex bg-white rounded-lg border border-[#2D3A3A]/10 p-1 shadow-sm" role="group" aria-label="Select Prediction Reference">
                  {["Moon", "Lagna"].map(r => <button key={r} aria-pressed={predictionReference === r} onClick={() => setPredictionReference(r as "Moon" | "Lagna")} className={`px-3 py-1 text-xs rounded-md transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#D97757] ${predictionReference === r ? 'bg-[#2D3A3A] text-white font-bold' : 'text-[#2D3A3A]/60 hover:bg-[#FDF8ED]'}`}>From {r}</button>)}
                </div>
              </div>
            </header>
            ...
            <div className="space-y-6">
              <h2 className="text-xs font-bold text-[#2D3A3A]/60 uppercase tracking-[0.3em] flex items-center gap-3 select-none">
                <div className="h-px w-8 bg-[#D97757]/40" /> Combined & Synthesized Insights
              </h2>
              <div className="grid grid-cols-1 gap-12">
                {(predictionReference === "Moon" ? predictionsMoon : predictionsLagna).combined?.map((pred, i) => {
                  ...
                  return (
                    <div key={i} className="bg-white p-8 md:p-10 rounded-[2.5rem] border border-[#2D3A3A]/10 shadow-sm hover:shadow-md transition-all border-t-4 border-t-[#D97757] grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                      ...
                      <div className="lg:col-span-7 space-y-6">
                        <div className="flex flex-wrap items-center justify-between gap-2 select-none">
                          ...
                          <div className="flex items-center gap-2">
                            <span className="text-[10px] font-bold uppercase tracking-wider text-[#2D3A3A]/50">Confidence:</span>
                            <span className={`text-xs font-extrabold px-2.5 py-0.5 rounded-full ${whyDetails.score >= 75 ? 'bg-green-100 text-green-800' : 'bg-[#D97757]/10 text-[#D97757]'}`}>
                              {whyDetails.score}%
                            </span>
                          </div>
                        </div>
                        ...
                        <div>
                          <h3 className="font-serif text-2xl md:text-3xl font-bold text-[#2D3A3A] mb-3">{pred.title}</h3>
                          <p className="text-sm md:text-base text-[#2D3A3A]/80 leading-relaxed">{pred.description}</p>
                        </div>

                        {/* Summary vs Detailed Toggle sections */}
                        {predictionsViewMode === "detailed" && (
                          <div className="space-y-4 pt-6 border-t border-[#2D3A3A]/5 bg-[#FDF8ED]/30 p-6 rounded-2xl">
                            <h4 className="text-xs font-extrabold text-[#2D3A3A] uppercase tracking-wider flex items-center gap-2 select-none">
                              <Sparkles className="w-3.5 h-3.5 text-[#D97757]" /> Why this prediction?
                            </h4>
                            <div className="space-y-2 text-xs">
                              <p className="font-semibold text-[#2D3A3A]/60">Breakdown & Key Signals:</p>
                              <ul className="list-disc pl-4 space-y-1 text-[#2D3A3A]/80 leading-relaxed">
                              ...
                              <div className="flex flex-wrap gap-2 pt-2">
                                {whyDetails.breakdown.map((b, bIdx) => (
                                  <span key={bIdx} className="bg-white/80 border border-[#2D3A3A]/10 px-2 py-1 rounded text-[10px] font-bold text-[#2D3A3A]/60">
                                    {b}
                                  </span>
                                ))}
                              </div>
                            ...
                            <div className="space-y-2 pt-4 border-t border-[#2D3A3A]/5 text-[10px]">
                              <p className="font-semibold text-[#2D3A3A]/40 uppercase tracking-widest">Calculation Assumptions & Base Model:</p>
                              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-[#2D3A3A]/60">
                                {whyDetails.assumptions.map((ass, aIdx) => (
                                  <div key={aIdx} className="flex items-center gap-1.5">
                                    <span className="w-1.5 h-1.5 rounded-full bg-[#2D3A3A]/20" />
                                    <span>{ass}</span>
                                  </div>
                                ))}
                              </div>
                        ...
                        {pred.contributors.length > 0 && (
                          <div className="pt-4 border-t border-[#2D3A3A]/5">
                            <div className="text-[10px] font-bold text-[#2D3A3A]/40 uppercase tracking-wider mb-2 select-none">Astrological Contributors</div>
                            <div className="flex flex-wrap gap-1.5">
                              {pred.contributors.map((contrib, cIdx) => (
                                <span key={cIdx} className="bg-[#2D3A3A]/5 px-2.5 py-1 text-[10px] font-bold text-[#2D3A3A]/70 rounded-full select-none">
                                  {contrib}
                                </span>
                              ))}
                            </div>
                      ...
                      <div className="lg:col-span-5 bg-[#FDF8ED]/30 p-6 rounded-3xl border border-[#2D3A3A]/5 space-y-6">
                        <div className="flex justify-between items-center pb-2 border-b border-[#2D3A3A]/10 select-none">
                          <div>
                            <h4 className="text-xs font-bold text-[#2D3A3A] uppercase tracking-wider">Supporting Reference Charts</h4>
                            <p className="text-[10px] text-[#2D3A3A]/50">Houses {targetHighlightHouses.join(", ")} Highlighted</p>
                          </div>
                          <div className="flex bg-white rounded-lg border border-[#2D3A3A]/10 p-1" role="group" aria-label="Select Prediction Chart Style">
                            {["North", "South"].map(s => (
                              <button
                                key={s}
                                aria-pressed={chartStyle === s}
                                onClick={() => setChartStyle(s as ChartStyle)}
                                className={`px-2.5 py-0.5 text-[10px] rounded transition-all font-semibold ${chartStyle === s ? 'bg-[#D97757] text-white shadow-sm' : 'text-[#2D3A3A]/60 hover:bg-[#FDF8ED]'}`}
                              >
                                {s}
                              </button>
                            ))}
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          ...
                          <div className="space-y-2 relative">
                            <div className="text-center text-[10px] font-extrabold text-[#D97757] uppercase tracking-widest">
                              Natal (Rotated)
                            </div>
                            <div className="p-2 bg-white rounded-2xl shadow-sm border border-[#2D3A3A]/5 hover:scale-[1.02] transition-all cursor-pointer relative group">
                              {predictionNatalChart ? (
                                <div onClick={() => setExpandedChart({ data: predictionNatalChart, title: `Natal: ${pred.type} Supporting Chart`, highlightedHouses: targetHighlightHouses })}>
                                  ...
                                  <div className="absolute inset-0 bg-black/5 opacity-0 group-hover:opacity-100 transition-opacity rounded-2xl flex items-center justify-center">
                                    <span className="text-[10px] bg-white px-2 py-1 rounded shadow text-[#2D3A3A] font-bold">Maximize Chart</span>
                                  </div>
                                </div>
                              ) : (
                                <div className="aspect-square w-full bg-[#FDF8ED]/20 rounded-xl flex items-center justify-center text-[10px] text-center p-2 text-[#2D3A3A]/40 font-bold">
                                  No Natal Data
                                </div>
                              )}
                            </div>
                          ...
                          <div className="space-y-2 relative">
                            <div className="text-center text-[10px] font-extrabold text-[#2D3A3A]/60 uppercase tracking-widest">
                              Transit (Gochara)
                            </div>
                            <div className="p-2 bg-white rounded-2xl shadow-sm border border-[#2D3A3A]/5 hover:scale-[1.02] transition-all cursor-pointer relative group">
            ...
            <div className="space-y-6">
              <h2 className="text-xs font-bold text-[#2D3A3A]/60 uppercase tracking-[0.3em] flex items-center gap-3 select-none">
                <div className="h-px w-8 bg-[#D97757]/40" /> Major Influences
              </h2>
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                {planets.filter(p => ["Sun", "Moon", dasha?.mahadasha.lord].includes(p.name)).map((p, i) => (
                    <div key={i} className="bg-white p-6 rounded-[2rem] border border-[#2D3A3A]/10 shadow-sm hover:shadow-md transition-all border-l-4 border-l-[#D97757]">
                        <div className="flex justify-between items-start mb-4">
                            <h3 className="font-serif text-lg font-bold">{p.name}</h3>
                            <span className="text-[10px] font-bold bg-[#FDF8ED] px-2 py-1 rounded border border-[#2D3A3A]/10 uppercase">{p.rasi}</span>
                        </div>
                        <p className="text-sm text-[#2D3A3A]/80 leading-relaxed mb-4">{activePlacementsMap.get(p.name) || 'Analyzing transit impact...'}</p>
            ...
            <div className="space-y-6">
              <h2 className="text-xs font-bold text-[#2D3A3A]/60 uppercase tracking-[0.3em] flex items-center gap-3 select-none">
                <div className="h-px w-8 bg-[#2D3A3A]/20" /> Supporting Transits
              </h2>
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 opacity-80">
                {planets.filter(p => ["Mars", "Mercury", "Jupiter", "Venus", "Saturn"].includes(p.name) && p.name !== dasha?.mahadasha.lord).map((p, i) => (
                    <div key={i} className="bg-white/60 p-6 rounded-3xl border border-[#2D3A3A]/10 shadow-sm hover:shadow-md transition-all">
                        <div className="flex justify-between items-start mb-4">
                            <h3 className="font-serif text-base font-bold">{p.name}</h3>
                            <span className="text-[10px] font-bold bg-[#FDF8ED] px-2 py-1 rounded border border-[#2D3A3A]/10 uppercase">{p.rasi}</span>
                        </div>
                        <p className="text-xs text-[#2D3A3A]/70 leading-relaxed mb-4 line-clamp-3">{activePlacementsMap.get(p.name) || 'Analyzing transit impact...'}</p>
                        <div className="flex gap-3">
            ...
            {(predictionReference === "Moon" ? predictionsMoon : predictionsLagna).yogas.length > 0 && (
                <section className="bg-[#2D3A3A] p-8 rounded-3xl text-white shadow-lg">
                    <h2 className="text-2xl font-serif mb-6 flex items-center gap-2 select-none"><Sparkles aria-hidden="true" className="text-[#D97757]"/> Active Planetary Yogas</h2>
                        ...
                        {(predictionReference === "Moon" ? predictionsMoon : predictionsLagna).yogas.map((y, i) => {
                            ...
                            return (
                                <div key={i} className="bg-white/10 p-4 rounded-xl border border-white/10">
                                    <div className="font-bold text-[#D97757] mb-1">{title}</div>
            ...
            <div className="flex justify-between items-end mb-4">
              <h2 className="text-xs font-bold text-[#2D3A3A]/60 uppercase tracking-[0.3em] flex items-center gap-3 select-none">
                <div className="h-px w-8 bg-[#2D3A3A]/20" /> Active Dasha Periods
              </h2>
              <span className="text-[10px] text-[#2D3A3A]/40 uppercase tracking-widest font-sans italic mr-2 select-none" title="Vimshottari lengths calculated using exact Tropical Year (365.24219 days) to align with standard classical/JHora expectations.">Tropical Year</span>
            </div>
            {dasha && (
                <section className="bg-white p-8 rounded-[2rem] border border-[#2D3A3A]/10 shadow-sm space-y-8 select-none">
                    {(() => {
                        const activeDate = getActiveTransitDate();
                        const activeTime = activeDate.getTime();

                        return [
                          { label: "Mahadasha", value: dasha.mahadasha, color: "bg-[#D97757]", textColor: "text-[#D97757]" },
                          { label: "Antardasha", value: dasha.antardasha, color: "bg-[#2D3A3A]", textColor: "text-[#2D3A3A]" },
                          ...
                          { label: "Sookshma Dasha", value: dasha.sookshmadasha, color: "bg-teal-500", textColor: "text-teal-600" }
                            ...
                            // Status badge
                            let status = "Upcoming";
                            let statusBg = "bg-[#2D3A3A]/5 text-[#2D3A3A]/50";
                            ...
                            return (
                                <div key={level.label} className="p-6 rounded-2xl border border-[#2D3A3A]/5 bg-[#FDF8ED]/30 hover:bg-[#FDF8ED]/50 transition-all">
                                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4">
                                        <div>
                                            <div className="flex items-center gap-2">
                                                <span className="text-xs font-bold text-[#2D3A3A]/40 uppercase tracking-widest">{level.label}</span>
                                                <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold uppercase tracking-wider ${statusBg}`}>{status}</span>
                                            </div>
                                            <h3 className="text-2xl font-serif font-bold text-[#2D3A3A] mt-1">
                                                <span className={level.textColor}>{level.value.lord}</span> Period
                                            </h3>
                                        </div>
                                        <div className="text-left md:text-right text-xs text-[#2D3A3A]/60">
                                            <div><span className="font-bold">Range:</span> {start.toLocaleDateString()} &mdash; {end.toLocaleDateString()}</div>
                                            <div className="mt-1 font-medium text-[#2D3A3A]/40">Total: {totalStr}</div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="space-y-2">
                                        <div
                                            className="h-2.5 w-full bg-[#FDF8ED] rounded-full overflow-hidden border border-[#2D3A3A]/5 relative"
                                            role="progressbar"
                                        ...
                                        <div className="flex justify-between text-[11px] font-medium text-[#2D3A3A]/60">
                                            <div>
                                                <span className="text-[#2D3A3A]/40">Elapsed:</span> {elapsedStr} ({percentage.toFixed(1)}%)
                                            </div>
                                            <div>
                                                <span className="text-[#2D3A3A]/40">Remaining:</span> {remainingStr}
                                            </div>
                                        </div>
                                    </div>
                                </div>
            ...
            <h2 className="text-xs font-bold text-[#2D3A3A]/60 uppercase tracking-[0.3em] flex items-center gap-3 select-none">
              <div className="h-px w-8 bg-[#2D3A3A]/20" /> Astrological Remedies (Upayas)
            </h2>
            <div className="grid md:grid-cols-2 gap-6">
                {remedies.map((remedy, i) => (
                    <div key={i} className="bg-white p-8 rounded-[2rem] border border-[#2D3A3A]/10 shadow-sm group hover:border-[#D97757]/40 transition-all">
                        <div className="flex justify-between items-start mb-6">
                            <h3 className="text-xl font-serif font-bold text-[#2D3A3A]">{remedy.planet} Upaya</h3>
                            <span className="text-[10px] font-bold text-[#D97757] bg-[#D97757]/5 px-3 py-1 rounded-full border border-[#D97757]/20 uppercase">{remedy.condition}</span>
                        </div>
                        <div className="space-y-6">
                            <div className="bg-[#FDF8ED] p-4 rounded-2xl border border-[#2D3A3A]/5">
                                <div className="text-[10px] font-bold text-[#D97757] uppercase mb-2 tracking-widest">Sattvic Mantra</div>
                                <p className="font-serif italic text-[#2D3A3A] leading-relaxed">&ldquo;{remedy.mantra}&rdquo;</p>
                            </div>
                            <div>
                                <div className="text-[10px] font-bold text-[#2D3A3A]/40 uppercase mb-1 tracking-widest">Charity (Daan)</div>
                                <p className="text-sm text-[#2D3A3A]/80">{remedy.charity}</p>
                            </div>
                            <div>
                                <div className="text-[10px] font-bold text-[#2D3A3A]/40 uppercase mb-1 tracking-widest">Lifestyle</div>
                                <p className="text-sm text-[#2D3A3A]/80">{remedy.lifestyle}</p>
                            </div>
                        </div>
                    </div>
                ))}
        ...
        {activeTab === "soul" && soulPurpose && (
          <div className="space-y-8 animate-in fade-in duration-500">
            <div className="bg-gradient-to-r from-[#2D3A3A] to-[#2E4343] text-white p-8 md:p-10 rounded-[2.5rem] shadow-xl space-y-4 select-none relative overflow-hidden">
              <div className="absolute right-0 top-0 translate-x-12 -translate-y-12 w-64 h-64 rounded-full bg-white/5 blur-3xl" />
              <div className="flex items-center gap-3">
                <div className="w-12 h-12 rounded-2xl bg-white/10 flex items-center justify-center">
                ...
                <div>
                  <h1 className="text-3xl md:text-4xl font-serif tracking-tight text-white font-bold">Dharma & Soul Purpose</h1>
                  <p className="text-white/70 text-sm mt-1 max-w-xl">Deep insights into your karmic path, innate strengths, and meditative guidance based on your natal chart.</p>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="bg-white p-6 rounded-3xl shadow-sm border border-[#2D3A3A]/5 hover:shadow-md transition-all">
                      ...
                      {soulPurpose.dharmaHouse.occupants.length > 0 && (
                          <div className="mt-2 text-xs text-[#2D3A3A]/60 font-bold bg-[#FDF8ED] inline-block px-2 py-1 rounded">Occupied by: {soulPurpose.dharmaHouse.occupants.join(", ")}</div>
                      )}
                  </div>
                  <div className="bg-white p-6 rounded-3xl shadow-sm border border-[#2D3A3A]/5 hover:shadow-md transition-all">
                      <div className="text-[10px] font-bold text-[#2D3A3A]/40 uppercase tracking-widest mb-1">The Sun (Your Vital Core)</div>
                      <div className="text-xl font-serif text-[#D97757] font-bold mb-2">House {soulPurpose.sunPlacement.house} in {soulPurpose.sunPlacement.sign}</div>
                      <p className="text-sm text-[#2D3A3A]/80">Focus: {soulPurpose.sunPlacement.theme.toLowerCase().replace('.', '')}</p>
                  </div>
                  <div className="bg-white p-6 rounded-3xl shadow-sm border border-[#2D3A3A]/5 hover:shadow-md transition-all">
                      <div className="text-[10px] font-bold text-[#2D3A3A]/40 uppercase tracking-widest mb-1">The Moon (Your Emotional Center)</div>
                      <div className="text-xl font-serif text-[#D97757] font-bold mb-2">House {soulPurpose.moonPlacement.house} in {soulPurpose.moonPlacement.sign}</div>
                      <p className="text-sm text-[#2D3A3A]/80">Comfort: {soulPurpose.moonPlacement.theme.toLowerCase().replace('.', '')}</p>
                  </div>
              </div>
            </div>
            ...
            <div className="space-y-6">
              <h2 className="text-xs font-bold text-[#2D3A3A]/60 uppercase tracking-[0.3em] flex items-center gap-3 select-none">
                <div className="h-px w-8 bg-[#D97757]/40" /> Synthesis & Guidance
              </h2>
              <div className="bg-white p-8 md:p-10 rounded-[2rem] shadow-sm border border-[#2D3A3A]/5 space-y-8">
                  <div>
                      <h3 className="text-xl font-serif text-[#2D3A3A] font-bold mb-4 flex items-center gap-2"><Sparkles className="w-5 h-5 text-[#D97757]" /> Karmic Path</h3>
                      <p className="text-[#2D3A3A]/80 leading-relaxed">{soulPurpose.karmicPath}</p>
                  </div>
                  <div className="w-full h-px bg-gradient-to-r from-transparent via-[#2D3A3A]/10 to-transparent" />
                  <div>
                      <h3 className="text-xl font-serif text-[#2D3A3A] font-bold mb-4 flex items-center gap-2"><Sun className="w-5 h-5 text-[#D97757]" /> Zen Meditative Guidance</h3>
                      <p className="text-[#2D3A3A]/80 leading-relaxed text-lg italic bg-[#FDF8ED] p-6 rounded-2xl border-l-4 border-[#D97757]">
                          &quot;{soulPurpose.zenGuidance}&quot;
                      </p>
                  </div>
              </div>
            </div>
        ...
        {activeTab === "charts" && (
            <div className="space-y-8 animate-in fade-in duration-500">
                ...
                <div className="bg-gradient-to-r from-[#2D3A3A] to-[#142D31] text-white p-8 md:p-10 rounded-[2.5rem] shadow-xl space-y-4 select-none relative overflow-hidden">
                  <div className="absolute right-0 top-0 translate-x-12 -translate-y-12 w-64 h-64 rounded-full bg-white/5 blur-3xl" />
                  <div className="flex items-center gap-3">
                    <div className="w-12 h-12 rounded-2xl bg-white/10 flex items-center justify-center">
                      <Info className="w-6 h-6 text-[#D97757]" />
                    </div>
                    <div>
                  ...
                  <div className="flex items-center gap-2 text-xs text-[#D97757] font-bold uppercase tracking-wider pt-4 border-t border-white/10">
                ...
                <header className="flex flex-col md:flex-row justify-between items-center gap-4 select-none">
                    <h2 className="text-2xl font-serif text-[#2D3A3A] font-bold">Diagnostics Workspace</h2>
                    <div className="flex flex-wrap gap-4">
                        <div className="flex bg-white rounded-lg border border-[#2D3A3A]/10 p-1 shadow-sm" role="group" aria-label="Select Chart Style">
                            {["North", "South"].map(s => <button key={s} aria-pressed={chartStyle === s} onClick={() => setChartStyle(s as ChartStyle)} className={`px-4 py-1 text-xs rounded-md transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#D97757] ${chartStyle === s ? 'bg-[#D97757] text-white font-bold' : 'text-[#2D3A3A]/60 hover:bg-[#FDF8ED]'}`}>{s} Indian</button>)}
                        </div>
                        <div className="flex bg-white rounded-lg border border-[#2D3A3A]/10 p-1 shadow-sm">
                            <button
                                aria-pressed={showUpgrahasInCharts}
                                onClick={() => setShowUpgrahasInCharts(!showUpgrahasInCharts)}
                                className={`px-4 py-1 text-xs rounded-md transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#D97757] ${showUpgrahasInCharts ? 'bg-[#2D3A3A] text-white font-bold' : 'text-[#2D3A3A]/60 hover:bg-[#FDF8ED]'}`}
                            >
                ...
                <div className="grid lg:grid-cols-2 gap-8">
                    <div className="bg-white p-6 rounded-3xl border border-[#2D3A3A]/10 shadow-sm">
                        <h2 className="text-center font-serif text-lg mb-6 text-[#D97757] select-none">Natal Chart (Birth)</h2>
                        {natalChart ? (
                            <KundliChart
                                data={showUpgrahasInCharts ? (natalChartWithUpgrahas || natalChart) : natalChart}
                                style={chartStyle}
                            />
                        ) : <div className="h-64 bg-[#FDF8ED] rounded animate-pulse flex items-center justify-center text-xs opacity-40 italic">Set birth time to see natal chart...</div>}
                    </div>
                    <div className="bg-white p-6 rounded-3xl border border-[#2D3A3A]/10 shadow-sm ring-2 ring-[#D97757]/20">
                        <h2 className="text-center font-serif text-lg mb-6 text-[#2D3A3A] select-none">Transit Chart (Now)</h2>
                        {chartData ? (
                            <KundliChart
                                data={showUpgrahasInCharts ? (d1WithUpgrahas || chartData) : chartData}
                                style={chartStyle}
                            />
                        ) : <div className="h-64 bg-[#FDF8ED] rounded animate-pulse" />}
                    </div>
                </div>

                <div className="grid lg:grid-cols-2 gap-8">
                    {[{t: "D9 Navamsa", d: d9Data, du: d9WithUpgrahas}, {t: "D60 Shashtiamsa", d: d60Data, du: d60WithUpgrahas}].map((c, i) => (
                        <div key={i} className="bg-white p-6 rounded-3xl border border-[#2D3A3A]/10 shadow-sm">
                ...
                {upgrahas.length > 0 && (
                    <section className="bg-white p-8 rounded-3xl border border-[#2D3A3A]/10 shadow-sm">
                        <h2 className="text-2xl font-serif mb-8 text-[#2D3A3A] select-none">Upgrahas (Shadow Planets & Sub-Planets)</h2>
                        <div className="overflow-x-auto">
                            <table className="w-full text-left text-sm">
                                <thead className="border-b border-[#2D3A3A]/10">
                                    <tr className="text-[#2D3A3A]/40 font-bold text-xs uppercase">
                                ...
                                <tbody className="divide-y divide-[#2D3A3A]/5">
                                    {upgrahas.map((u) => (
                                        <tr key={u.name} className="group hover:bg-[#FDF8ED]/50">
                                            <td className="py-4 font-bold text-[#2D3A3A]">{u.name}</td>
                                            <td className="py-4"><span className="px-2.5 py-1 bg-[#2D3A3A]/5 border border-[#2D3A3A]/10 text-xs font-serif rounded text-[#2D3A3A] font-bold select-none">{u.symbol}</span></td>
                ...
                {ashtakavarga && (
                    <section className="bg-white p-8 rounded-3xl border border-[#2D3A3A]/10 shadow-sm">
                        <h2 className="text-2xl font-serif mb-8 text-[#2D3A3A] select-none">Ashtakavarga Analysis (SAV)</h2>
                        <div className="grid grid-cols-4 md:grid-cols-6 lg:grid-cols-12 gap-3 mb-10">
                            {ashtakavarga.sav.map((s, i) => (
                                <div key={i} className={`p-4 rounded-2xl border text-center transition-all ${s >= 28 ? 'bg-green-50 border-green-200' : s <= 25 ? 'bg-red-50 border-red-200' : 'bg-[#FDF8ED] border-[#2D3A3A]/10'}`}>
                                    <div className="text-[10px] font-bold text-[#2D3A3A]/40 uppercase mb-1">{["Ari", "Tau", "Gem", "Can", "Leo", "Vir", "Lib", "Sco", "Sag", "Cap", "Aqu", "Pis"][i]}</div>
                                    <div className={`text-xl font-bold ${s >= 28 ? 'text-green-700' : s <= 25 ? 'text-red-700' : 'text-[#2D3A3A]'}`}>{s}</div>
                                </div>
                            ))}
                        </div>
                        <div className="overflow-x-auto">
                            <table className="w-full text-left text-sm">
                                <thead className="border-b border-[#2D3A3A]/10"><tr className="text-[#2D3A3A]/40 font-bold text-xs uppercase"><th className="pb-4">Planet</th>{["Ari", "Tau", "Gem", "Can", "Leo", "Vir", "Lib", "Sco", "Sag", "Cap", "Aqu", "Pis"].map(s => <th key={s} className="pb-4">{s}</th>)}</tr></thead>
                                <tbody className="divide-y divide-[#2D3A3A]/5">
                                    {Object.entries(ashtakavarga.bav).map(([p, scores]) => (
                                        <tr key={p} className="group hover:bg-[#FDF8ED]/50"><td className="py-4 font-bold text-[#2D3A3A]">{p}</td>{scores.map((s, idx) => <td key={idx} className={`py-4 ${s >= 5 ? 'text-green-600 font-bold' : s <= 3 ? 'text-red-500' : 'text-[#2D3A3A]/60'}`}>{s}</td>)}</tr>
                                    ))}
                                </tbody>
                            </table>
                        </div>
      ...
      {expandedChart && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm select-none" onClick={() => setExpandedChart(null)}>
          <div className="bg-white p-6 md:p-8 rounded-[2.5rem] border border-[#2D3A3A]/10 shadow-2xl max-w-lg w-full space-y-6 relative" onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-center border-b border-[#2D3A3A]/10 pb-4">
              <div>
                <h3 className="font-serif text-lg font-bold text-[#2D3A3A]">{expandedChart.title}</h3>
                {expandedChart.highlightedHouses && (
                  <p className="text-[10px] text-[#2D3A3A]/50 uppercase tracking-wider font-semibold">
              ...
              <button
                ...
                className="text-xs font-bold bg-[#2D3A3A]/5 text-[#2D3A3A]/60 hover:bg-[#2D3A3A]/10 px-3 py-1.5 rounded-full transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#D97757]"
              >
                Close
              </button>
            </div>
            <div className="p-4 bg-[#FDF8ED]/30 rounded-2xl border border-[#2D3A3A]/5">
              <KundliChart
      ...
      <nav aria-label="Main Navigation" role="tablist" className="fixed bottom-6 left-1/2 -translate-x-1/2 bg-[#2D3A3A]/90 backdrop-blur-md px-8 py-4 rounded-full flex gap-12 shadow-2xl items-center z-50 transition-all border border-white/10 select-none">
        {[{id: "dashboard", icon: Sun, label: "Sky"}, {id: "predictions", icon: Sparkles, label: "Predict"}, {id: "charts", icon: Info, label: "Deep"}, {id: "soul", icon: User, label: "Soul"}].map(tab => (
            <button
                key={tab.id} role="tab" aria-selected={activeTab === tab.id}
                onClick={() => setActiveTab(tab.id)}
                className={`flex flex-col items-center gap-1 p-2 -m-2 rounded-xl outline-none focus-visible:ring-2 focus-visible:ring-[#D97757] focus-visible:bg-white/5 transition-all ${activeTab === tab.id ? 'text-[#D97757] scale-110' : 'text-white/40 hover:text-white/80'}`}>
